...
Ai=(xs-xi).*hr; p=Ai/sum(Ai);
n=1000;
na=0; nt=0;
clear xP; clear yP
 while na<n
   % escolher retangulo
   [i]=selecao_aleatoria(p);
   
   nt=nt+1; % numero de tentativas
   xP(nt)=rand(1)*(xs(i)-xi(i))+xi(i); % xP uniforme entre xi(i) e xs(i)
...
%d  Metodo de transformação de variaveis para x gerado com
%distribuiçao gama
