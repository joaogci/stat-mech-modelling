...
beta_vals = 0.01:0.01:0.5;
n_beta = length(beta_vals);
frac = zeros(1, n_beta);

for i = 1:n_beta
    beta=beta_vals(i); gama=1; ntransicoes=1e5; 
    % fazer a simulação
...
figure(1)
plot(beta_vals, frac, 'or')
xlabel("valores de beta")
ylabel("fração de sitios recuperados")
fprintf("há uma transição de fase perto de 0,35. por isso beta_C~0,35 \n");
